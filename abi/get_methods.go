...
var KnownGetMethodsDecoder = map[string][]func(tlb.VmStack) (string, any, error){
	...
	"get_vault_data":                               {DecodeGetVaultData_AffluentResult, DecodeGetVaultData_StonfiV2Result, DecodeGetVaultData_StormResult},
...
type GetVaultData_AffluentResult struct {
	AffluentVaultData struct {
		OwnerAddress                  tlb.MsgAddress
		Assets                        tlb.Any
		FactorialPools                tlb.Any
...
func GetVaultData(ctx context.Context, executor Executor, reqAccountID ton.AccountID) (string, any, error) {
	...
	for _, f := range []func(tlb.VmStack) (string, any, error){DecodeGetVaultData_AffluentResult, DecodeGetVaultData_StonfiV2Result, DecodeGetVaultData_StormResult} {
		s, r, err := f(stack)
		if err == nil {
			return s, r, nil
		}
...
func DecodeGetVaultData_AffluentResult(stack tlb.VmStack) (resultType string, resultAny any, err error) {
	if len(stack) != 1 || (stack[0].SumType != "VmStkTuple") {
		return "", nil, fmt.Errorf("invalid stack format")
	}
	var result GetVaultData_AffluentResult
	err = stack.Unmarshal(&result)
	return "GetVaultData_AffluentResult", result, err
}
